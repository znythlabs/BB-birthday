...
export function frameForClip(clock, clip, playing) {
  if (!Number.isInteger(clock.frame) || clock.frame < 0) {
    throw new RangeError("clock frame must be a non-negative integer");
  }
  if (!Number.isInteger(clip.frames) || clip.frames <= 0) {
    throw new RangeError("clip frames must be a positive integer");
  }
  if (!playing || clock.sheet !== clip.sheet) return 0;
  return Math.min(clock.frame, clip.frames - 1);
}
